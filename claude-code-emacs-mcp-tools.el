...
(defun claude-code-emacs-mcp-handle-get-diagnostics-resource (params)
  "Handle get-diagnostics-resource request for resources with PARAMS.
Returns diagnostics in resource format."
  (condition-case err
      (let ((buffer-path (cdr (assoc 'bufferPath params)))
            (diagnostics-hash (make-hash-table :test 'equal)))
        
        ;; If no LSP available, return empty diagnostics
        (unless (fboundp 'lsp-diagnostics)
          (if buffer-path
              `((success . t)
                (diagnostics . []))
            `((success . t)
              (diagnostics . ,diagnostics-hash))))
        
        ;; Get diagnostics from any buffer with LSP active
        (let ((lsp-buffer nil))
          ;; Find any buffer with LSP active
          (dolist (buf (buffer-list))
            (when (and (not lsp-buffer)
          ...
          (if lsp-buffer
              (let ((lsp-diags (with-current-buffer lsp-buffer
                                 (condition-case diag-err
                                     (lsp-diagnostics t)
                                   (error
                                    (message "Error getting LSP diagnostics: %s"
                                             (error-message-string diag-err))
                                    nil)))))
                (when lsp-diags
                  ;; Convert diagnostics to resource format
                  (maphash (lambda (file diags)
                             (let ((diag-list '()))
                               (dolist (diag diags)
                                 (let* ((message (lsp:diagnostic-message diag))
                                   ...
                                   (when (and file line)
                                     (push `((file . ,file)
                                             ...
                                             (source . ,(or source "lsp")))
                                           diag-list))))
                               (puthash file (nreverse diag-list) diagnostics-hash)))
                           lsp-diags))
                
                ;; Return result based on whether buffer-path was specified
                (if buffer-path
                    `((success . t)
                      (diagnostics . ,(gethash buffer-path diagnostics-hash [])))
                  `((success . t)
                    (diagnostics . ,diagnostics-hash))))
    ...
    (error
     ...
     (message "Error in get-diagnostics handler: %s" (error-message-string err))
     `((success . nil)
